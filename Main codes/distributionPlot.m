    ...
    switch opt.xyOri
        case 'normal'
            ...
            if opt.showMM == 6
                ...
                mdh(2,:) = plot(ah,squeeze(md(:,2,2:3))',repmat(md(:,2,1)',2,1),'color','k','lineWidth',0.5,'lineStyle','-');
                mdh(3,:) = plot(ah,squeeze(md(:,3,2:3))',repmat(md(:,3,1)',2,1),'color','k','lineWidth',0.5,'lineStyle','-');
            end
        case 'flipped'
            if any(opt.showMM==[1,2])
