...
modelFormula=['PV_correlation~Time*NeuronCategory +PostS*NeuronCategory+REM*NeuronCategory+size' ...
    '+(1 + PostS + REM + NeuronCategory|MouseID)'];

mixedEffectsModel = fitlme(dataTable, modelFormula, 'DummyVarCoding', 'reference', 'FitMethod', 'reml','CovariancePattern', 'Diagonal');
...
sz=repmat(numMice',[cd1,1]); %number of neurons per mice

REM=zeros(1,d2); REM(end)=1;REM=pdist(REM')';REM=repmat(REM,[1,cd2]);  % Wake (0) or REM sleep (1) session indicator
...
dataTable = table(categorical(mouse(:)), ...
    categorical(postS(:)), categorical(REM(:)), ...
    categorical(neuronCategory(:)),C(:),recordingDay(:),sz(:), ...
